...
def harris_corners(img, window_size=3, k=0.04):
    ...
    Iyy = dy**2
    Ixy = dx*dy
    pad_idx = window_size//2
    for i in range(pad_idx, H):
        for j in range(pad_idx, W):
            start_j = j - pad_idx
            start_i = i - pad_idx
            end_i = i + pad_idx
            end_j = j + pad_idx

            windowXX = Ixx[start_i:end_i, start_j:end_j] 
            windowYY = Iyy[start_i:end_i, start_j:end_j] 
            windowXY = Ixy[start_i:end_i, start_j:end_j]
...
def simple_descriptor(patch):
    ...
    for i in range(H):
        for j in range(W):
            nom = np.exp((-1/2)*np.square(patch[i, j]))
            den = np.sqrt(2*np.pi)
            if (den == 0):
                den = 1
            feature.append(nom / den)
...
def match_descriptors(desc1, desc2, threshold=0.5):
    ...
    ### YOUR CODE HERE
    for desc_idx, desc_dist in enumerate(dists):
        small_1, small_2 = np.sort(desc_dist)[0:2]
        idx_smallest_value = np.nonzero(desc_dist == small_2)[0][0]
        rt = small_1 / small_2
        if rt >= threshold:
            continue
        matches.append([desc_idx, idx_smallest_value])
...
def ransac(keypoints1, keypoints2, matches, n_iters=200, threshold=20):
    ...
    for i in range(n_iters):
        ...
        affine_transformation_matrix, _, _, _ = np.linalg.lstsq(random_matched2, random_matched1,rcond=None)
        ...
        for j in range(c):
            for z in range(pixels_per_cell[0]):
                for t in range(pixels_per_cell[1]):
                    ...
                    lBN = (int(np.trunc(t_C[i][j][z][t] / degrees_per_bin))) % 9
                    uBN = (lBN + 1) % 9
                    cells[i][j][uBN] += (division_ratio*g_C[i][j][z][t])
                    cells[i][j][lBN] += ((1-division_ratio) * g_C[i][j][z][t])
